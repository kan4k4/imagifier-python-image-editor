...
# logo = ImageTk.PhotoImage(file='imagifier_logo.png')
...
# logo = Image.open('imagifier_logo.png')
# # label = tk.Label(root,text="Imagifier II")
# render = ImageTk.PhotoImage(logo)
# img = tk.Label(root, image=render)
# img.image = render
# # img.grid(row=0,column=3, columnspan=6)
# img.place(x=700, y=600)

# label1 = tk.Label(root,text="Filters")
# label1.config(font=('helvetica', 18))
# label1.grid(row=1,column=12, columnspan=5)
...
def getPhoto ():
    ...
    # tk.Label(image = img).place(canvas)
    # canvas = tk.Label(image=img)
    # canvas.image = img
    # photo.show()
...
def manganify ():
    ...
    # enhancer = ImageEnhance.Brightnes(result)
    # factor = 0.8
    # result = enhancer.enhance(factor)
    # canvas.image = ImageTk.PhotoImage(result )
    # canvas.create_image(300,180, image=canvas.image, anchor='center')
...
def brightnessUp ():
    ...
    result = enhancer.enhance(factor)
    # result = photo
    # size = (result.size[0]/2,  result.size[1]/2)
    # result.thumbnail(size)
    canvas.image = ImageTk.PhotoImage(result)
    canvas.create_image(410,250, image=canvas.image, anchor='center')
